...
module.exports.postValidation = (req, res, next) => {
    ...
    if((req.body.bathroom && req.body.bathroom !== true) ||
        (req.body.bathroom && req.body.bathroom !== false)){
        errors.push('Bathroom must be boolean.')
    }
    //kitchen
    if(!req.body.kitchen){ errors.push('Bathroom is required.')}
    if((req.body.kitchen && req.body.kitchen !== true) ||
        (req.body.kitchen && req.body.kitchen !== false)){
        errors.push('Kitchen must be boolean.')
    }
    //aircon
    if(!req.body.air_con){ errors.push('Air conditioner is required.')}
    if((req.body.air_con && req.body.air_con !== true) ||
        (req.body.air_con && req.body.air_con !== false)){
        errors.push('Air conditioner must be boolean.')
    }
    //waterheater
    if(!req.body.water_heater){ errors.push('Water heater is required.')}
    if((req.body.water_heater && req.body.water_heater !== true) ||
        (req.body.water_heater && req.body.water_heater !== false)){
    ...
    if(req.body.time && !req.body.title.match(/^[1-4]{1}$/)){
